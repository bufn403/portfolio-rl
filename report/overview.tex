...
Over the full trading period (2010-2015), we will feed all of the headlines and SEC filings items to pre-trained FinBERT. 
The model then will preprocess the text and generate probabilities of the content 
being positive, negative, or neutral. From there, we can assign each textual sample a 
...
Thus, to fill the gaps between SEC filings dates, we apply exponential decay to the sentiment scores on report dates. That is we assume that the weight of our SEC sentiment embedding decays by some constant rate $\lambda$ over time. Formally,

$$
\frac{dN}{dt} = -\lambda N
$$

Where $N$ represents the company's sentiment score for the day, $t$ represents time (in days), and $\lambda$ is a positive constant representing the daily rate of decay. We test $\lambda = 0.1, 0.5, 0.9$ in our models to see what performs optimally. Note that we test a wide range of $\lambda$ values because we are unsure of how fast the signal from SEC filings decay.
