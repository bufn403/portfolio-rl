...
def read_news_data(date_range: pd.DatetimeIndex, gamma: float = 1.0) -> pd.DataFrame:
  ...
  min_date = min(news_df.fdate.min(), date_range.min())
  max_date = max(news_df.fdate.max(), date_range.max())
  date_range = pd.date_range(min_date, max_date, freq='D')
  ...
  for ticker in news_df.Ticker.unique():
      ...
      full_df.loc[full_df.ticker == ticker, 'sentiment_embedding'] = ticker_df.Tone.values
  full_df.sentiment_score = full_df.sentiment_embedding.fillna(0.0)
