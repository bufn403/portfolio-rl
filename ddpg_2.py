...
class Custom_EIEE_Network(nn.Module):
    def __init__(self, observation_space: gym.spaces.Dict, input_features: int = 6, *args, **kwargs):
        ...
        torch.nn.init.uniform_(self.net[4].weight, a=-0.3, b=0.5)
...
class DenseContinuousCritic(BaseModel):
    def __init__(
        ...
        for idx in range(n_critics):
            # q_net = DenseMlp(85, 1)
            q_net = nn.Sequential(
                Custom_EIEE_Network(observation_space, input_features=7),
                # nn.Softmax(dim=1),
                nn.Linear(85, 1)
            )
...
train_env = PortfolioEnvWithTCost(dm=TrainDataManager(), rm=ProfitReward(), cp=0.01, cs=0.01, w_lb=-2, w_ub=4)
